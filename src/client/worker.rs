...
impl NakadiWorker {
    /// Creates a new instance. The returned `JoinHandle` can be used to synchronize with the underlying worker thread.
    /// The underlying worker will be stopped once the worker is dropped.
    pub fn new<C: NakadiConnector, H: Handler>(connector: Arc<C>, handler: H, subscription_id: SubscriptionId) -> (NakadiWorker, JoinHandle<()>) {
        let is_running = Arc::new(AtomicBool::new(true));

        let handle = start_nakadi_worker_loop(connector.clone(), handler, subscription_id.clone(), is_running.clone());
...
fn nakadi_worker_loop<C: NakadiConnector, H: Handler>(connector: &C, handler: H, subscription_id: &SubscriptionId, is_running: Arc<AtomicBool>) {
    while (*is_running).load(Ordering::Relaxed) {
        ...
        for line in buffered_reader.lines() {
            match line {
                Ok(line) => {
                    match process_line(connector, line.as_ref(), &handler, &stream_id, subscription_id, &is_running) {
...
fn connect<C: ReadsStream>(connector: &C, subscription_id: &SubscriptionId, is_running: &AtomicBool) -> Option<(C::StreamingSource, StreamId)> {
    let mut attempt = 0;
    while is_running.load(Ordering::Relaxed) {
        attempt += 1;
...
fn checkpoint<C: Checkpoints>(checkpointer: &C, stream_id: &StreamId, subscription_id: &SubscriptionId, cursors: &[Cursor], is_running: &AtomicBool) {
    let mut attempt = 0;
