    ...
    B: BuildHasher + Clone,
{
    ...
    pub fn new_sorted_with_hasher(mut partitions: Vec<PartitionId>, build_hasher: B) -> Self {
        partitions.sort();
        Self::new_with_hasher(partitions, build_hasher)
    }
        ...
        C: MonitoringApi,
    {
        ...
        let partition_ids: Vec<PartitionId> = partitions.into_iter().map(|p| p.partition).collect();

        let ids_and_ints: Result<Vec<_>, _> = partition_ids
            .iter()
            .map(|id| id.as_str().parse::<u64>().map(|n| (id, n)))
            .collect();

        let partitioner = if let Ok(mut ids_and_ints) = ids_and_ints {
            ...
            Partitioner::new_sorted_with_hasher(partition_ids, build_hasher)
        };
