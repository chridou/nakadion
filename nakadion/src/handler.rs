...
use crate::model::{BatchMetadata, PartitionId};
...
impl ProcessingStatus {
    ...
    /// Processing events failed.
    pub fn failed() -> ProcessingStatus {
...
pub trait BatchHandler {
    /// Handle the events.
    ///
    /// Calling this method must not panic!
    ...
    fn handle(&mut self, events: &[u8], meta: BatchMetadata) -> ProcessingStatus {
        match serde_json::from_slice::<Vec<E>>(events) {
            Ok(events) => {
                ...
                match TypedBatchHandler::handle(self, events, meta) {
