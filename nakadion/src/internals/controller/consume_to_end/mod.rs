    ...
    S: Stream<Item = Result<BatchLineMessage, BatchLineError>> + Send + 'static,
{
    ...
    let mut consume_data = ConsumeData::new(stream_state.clone());
    ...
    match (loop_result, shut_down_result) {
        (Ok(()), Ok(sleeping_dispatcher)) => {
            stream_state.info(format_args!(
                ...
                consume_data.stream_started_at.elapsed(),
            ));
...
async fn handle_batch_line(
    ...
    if batch_line.is_keep_alive_line() {
        ...
        consume_data.last_events_received_at = now;
            ...
            .is_err()
        {
            ...
            consume_data.batches_sent_to_dispatcher += 1;
            Ok(())
        }
    }
}
...
async fn handle_tick(
    ...
    if consume_data.stream_dead_policy.is_stream_dead(
        consume_data.last_frame_received_at,
        consume_data.last_events_received_at,
    ) {
        return Err(Error::new("The stream is dead boys..."));
    }

    let elapsed = consume_data.last_frame_received_at.elapsed();
    if elapsed >= consume_data.warn_no_frames {
    ...
    let elapsed = consume_data.last_events_received_at.elapsed();
    if elapsed >= consume_data.warn_no_events {
    ...
    S: Stream<Item = Result<BatchLineMessage, BatchLineError>> + Send + 'static,
{
    ...
    let (result, unprocessed_batches) = match active_dispatcher.join().await {
        Ok(EnrichedOk {
            ...
            Ok(sleeping_dispatcher),
            Some(consume_data.batches_sent_to_dispatcher - processed_batches),
        ),
        Err(EnrichedErr {
            processed_batches,
            err,
